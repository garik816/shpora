...
unsigned char upDown=1;     // начинаем с движени€ вверх
unsigned char cylon=0;      // определ€ет очередность LED
//unsigned char Arnd;
...
ISR(TIMER0_OVF_vect)			//обработчик прерывани€
{
   /*перезапись счетного регистра*/
   TCNT0 = T_POLL;

   /*опрос кнопки*/
	if (bit_is_clear(PINC,PC0)) // если кнопка нажата (подключена к vcc, замыкаетс€ на gnd)
		{
		PORTD &= (~(1<<7));		//зажигаем
			 _delay_ms(5);
		PORTD |= (1<<7);		//гасим
